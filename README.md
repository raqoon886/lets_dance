...
Let's Dance는 싱글 플레이뿐만 아니라 두 명의 플레이어가 한 화면에서 동시에 춤을 추며 경쟁할 수 있는 로컬 멀티플레이 모드를 지원합니다.

- **다중 인원 포즈 감지**: 단일 카메라 피드 내에서 최대 2명의 플레이어를 분리해 동작을 독립적으로 감지하고 채점합니다.
- **분할 화면 UI**: 플레이어 간 경쟁 몰입도를 높일 수 있도록 플레이어별 점수, 콤보, 실시간 오버레이(스켈레톤 및 판정 이펙트)가 좌/우로 분할 표시됩니다.
- **실시간 경쟁 기믹**: 곡 종료 시 두 플레이어의 정확도와 스코어를 비교하여 최종 승패와 랭킹을 결과 화면에 노출합니다.
